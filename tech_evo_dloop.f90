...
program igs
  ...
  ! Set parameters
  q = 0.5_dp; N0 = 100.0_dp; E0 = 10.0_dp; tm = 0.5_dp; gm = 0.5_dp

  d = 0.0_dp

  outunit = 40
  write(filename, '(A,I0,A)') 'output_q_05_nt_', nt, '.dat'
  open(unit=outunit, file=filename, status='unknown')
  write(outunit, '(A,I0)') '# Seed: ', idum
  write(outunit, '(A,2F8.2,1X,F8.4,1X,I0)') '# Parameters: E0, N0, q, nt = ', E0, N0, q, nt
  write(outunit, '(A)') '# Format: d, E/E0, N/N0, tau, t(ns), ns'

1 nreal = 100 ! Number of realizations per d
...
10 ns = ns + 1
  ...
  if (1.0_dp - q*Chi > 0) write(outunit,*) d, E / E0, N / N0, tau, t(ns), ns
  nreal = nreal - 1
  if (nreal > 1) goto 5
  if (d < 10.0) goto 1
