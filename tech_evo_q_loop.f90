! Fortran program to simulate technology evolution - Parameter sweep over q
! Loops over q values from 0 to 1 with 100 values
! Tests nt = 10^4, 10^5 and 10^6
! Saves only final values per q in separate files for each nt
! By Debora Princepe - 2025-07-16
! run: gfortran -O3 -march=native -funroll-loops -ffast-math tech_evo_q_loop.f90 -o qloop 

program tech_evo_q_loop
    implicit none

    integer, parameter    :: dp = selected_real_kind(15)
    integer, parameter    :: n_q_values = 100
    integer, parameter    :: n_nt_values = 2
    integer, dimension(n_nt_values), parameter :: nt_array = [10000, 100000]
    ...
    integer :: seed_size, i, j, k, unit, ns, iter, max_iter = 10000
    integer :: nt, nt_idx, q_idx
    logical :: converged_E
    real(dp) :: mean = 0.5_dp
    ...
    ! Loop parameters
    real(dp) :: q_min = 0.0_dp, q_max = 1.0_dp, q_step
    ...
    ! File units for output (separate for each nt)
    integer :: unit_final, unit_aggregates
    
    ! Timing variables
    real(dp) :: cpu_start, cpu_end, elapsed_time

    ! Read parameters (d, E0, N0 only - q will be looped)
    num_args = command_argument_count()
    if (num_args /= 3) then
        print *, "Use: ./qloop d E0 N0"
        print *, "Example: ./qloop 0.8 10.0 1000.0"
        stop
    end if
    ...
    q_step = (q_max - q_min) / real(n_q_values - 1, dp)

    ! Seed handling
    call handle_random_seed(seed, seed_filename, use_saved_seed)

    print *, "Starting parameter sweep over q with nt values: ", nt_array

    ! Main loops: nt values, then q values
    do nt_idx = 1, n_nt_values
        nt = nt_array(nt_idx)
        
        print *, "=========================================="
        write(*, '(A,I0,A)') "Processing nt = ", nt
        print *, "=========================================="
        
        ! Create output files for this nt
        write(filename_final, '(A,I0,A,A,A)') 'final_values_nt_', nt, '_', trim(timestamp), '.dat'
        write(filename_agg, '(A,I0,A,A,A)') 'final_sums_nt_', nt, '_', trim(timestamp), '.dat'

        open(newunit=unit_final, file=filename_final, status='replace')
        write(unit_final, '(A,I0)') '# Final values per q for nt = ', nt
        write(unit_final, '(A,3F12.5)') '# Parameters: d, E0, N0 = ', d, E0, N0
        write(unit_final, '(A)') '# Format: q, ns_final, N_final, E_final, tau_final'

        open(newunit=unit_aggregates, file=filename_agg, status='replace')
        write(unit_aggregates, '(A,I0)') '# Sum per q for nt = ', nt
        write(unit_aggregates, '(A,3F12.5)') '# Parameters: d, E0, N0 = ', d, E0, N0
        write(unit_aggregates, '(A)') '# Format: q, Gamma, Theta, Delta, Phi, Psi, Chi, Sigma, Omega'
        ...
        do q_idx = 1, n_q_values
            q = q_min + real(q_idx - 1, dp) * q_step
            
            call cpu_time(cpu_start)
            
            ! Initialize for this q value
            num_valid = 0
            num_pending = 0
            ns = 0
            ...
            ! Write final values to files for this nt
            write(unit_final, '(F8.5,I8,3E16.8)') q, ns, N, E, Tau
            write(unit_aggregates, '(F8.5,8E16.8)') q, Gamma, Theta, Delta, Phi, Psi, Chi, Sigma, Omega

            ! Flush buffers to ensure data is written
            flush(unit_final)
            flush(unit_aggregates)

            ! Progress report
            if (mod(q_idx, 1) == 0 .or. q_idx <= 5) then
                write(*, '(A,F6.3,A,I0,A,F8.3,A)') &
                    "q=", q, ": ", ns, " technologies (", elapsed_time, "s)"
            end if
        end do
        
        ! Close files for this nt
        close(unit_final)
        close(unit_aggregates)
        
        write(*, '(A,I0,A)') "Completed nt = ", nt, " simulations"
        write(*, '(A,A)') "  Final values saved: ", trim(filename_final)
        write(*, '(A,A)') "  Sums saved:   ", trim(filename_agg)
        print *, ""
    end do
