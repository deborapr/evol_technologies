...
program igs
  ...
  outunit = 40
  write(filename, '(A,I0,A)') 'output_nt_', nt, '.dat'
  open(unit=outunit, file=filename, status='unknown')
  write(outunit, '(A,I0)') '# Seed: ', idum
  write(outunit, '(A,2F8.2,1X,F8.4,1X,I0)') '# Parameters: E0, N0, d, nt = ', E0, N0, d, nt
  write(outunit, '(A)') '# Format: q, E/E0, N/N0, tau, t(ns), ns'

1 nreal = 100 ! Number of realizations per q
...
10 ns = ns + 1
  ...
  if (1.0_dp - q*Chi > 0) write(outunit,*) q, E / E0, N / N0, tau, t(ns), ns
  nreal = nreal - 1
  if (nreal > 1) goto 5
  if (q < 1.0) goto 1
